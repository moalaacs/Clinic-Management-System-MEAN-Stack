    ...
    for (let i = 0; i < services.length; i++) {
      ...
      switch (patientType) {
        case "doctor":
          ...
          totalCost +=
            (clinicService.cost + services[i].additionalCosts) * doctorDiscount;
          let invoiceServicesObject1 = {
            name: clinicService.name,
            cost:
              (clinicService.cost + services[i].additionalCosts) *
              doctorDiscount,
          };
...
exports.allInvoicesReports = (request, response, next) => {
  invoiceSchema
    ...
    .populate({
      path: "clinic_Id",
      select: { _id: 0, _contactNumber: 1, _specilization: 1 }
    })
    .populate({ path: "patientId", select: { _id: 0, _fname: 1 } })
    .then((data) => {
